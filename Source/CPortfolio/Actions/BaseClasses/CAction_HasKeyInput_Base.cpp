#include "CAction_HasKeyInput_Base.h"
/*
#include "CHelpers.h"
#include "Notifies/CAN_EndAction.h"

void UCAction_HasKeyInput_Base::BeginPlay()
{
	Super::BeginPlay();
	...
	for(FActionData ActionData : ActionDatas)
	{
		for(FAnimNotifyEvent AnimNotify : ActionData.Montage->Notifies)
		{
			...
			{
				Cast<UCAN_EndAction>(AnimNotify.Notify)->OnEndAction.BindUObject(this, &UCAction_HasKeyInput_Base::EndAction);
			}
		}
	}
}

void UCAction_HasKeyInput_Base::SetDirectionToCamera() const
{
